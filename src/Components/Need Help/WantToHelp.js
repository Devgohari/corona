...
function WantToHelp() {
    ...
    let sendData = (e) => {
        e.preventDefault()
        console.log([{ name, email, mobile, city, address, age, service }]);
        state.helpgiver(service, name, email, mobile, age, address, city, message)
        setservice('');
        setname('')
        setemail('')
        setmobile('')
            ...
            <div className="flex border-2 rounded-lg border-gray-500 border-opacity-50 p-8 sm:flex-row flex-col">
                <div className="flex-grow">
                    ...
                    <form action="" method="get">
                        ...
                        <button onClick={sendData} type='submit' className="h-10 mt-2 px-5 text-indigo-100 transition-colors duration-150 bg-blue-600 rounded-lg focus:shadow-outline hover:bg-blue-800">Sumbit</button>
                    </form>
                </div>
            </div>
        </div>
